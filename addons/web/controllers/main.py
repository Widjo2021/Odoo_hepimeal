...
class Home(http.Controller):
    ...
    @http.route('/web/attandace', type='json', auth="none")
    def apiAttandance(self):
        request.session.db = "odoo"
        request.uid = odoo.SUPERUSER_ID
        data = request.params
        employee = request.env['hr.employee'].search(['|', ('work_email', '=', data['email']), ('private_email', '=', data['email'])])
        day = int(str(data['attendance_date'].split('-')[1]))
        month = int(str(data['attendance_date'].split('-')[2]))
        ...
        with registry.cursor() as cr:
            ...
            naive_ci = datetime.datetime.strptime(str(str(date) + " " + str(data['clock_in'])), "%Y-%m-%d %H:%M:%S")
            local_dt_ci = local.localize(naive_ci, is_dst=None)
            clock_in = str(local_dt_ci.astimezone(pytz.utc)).split("+")[0]

            clock_out = False
            if str(data['clock_out']) != '00:00:00':
                naive_co = datetime.datetime.strptime(str(str(date) + " " + str(data['clock_out'])), "%Y-%m-%d %H:%M:%S")
                local_dt_co = local.localize(naive_co, is_dst=None)
                clock_out = str(local_dt_co.astimezone(pytz.utc)).split("+")[0]

            cr.execute("SELECT * FROM hr_attendance WHERE check_in::date = '" + str(date) + "' and check_out is null  and employee_id = "+str(employee.id))
