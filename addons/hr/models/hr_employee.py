...
class HrEmployeePrivate(models.Model):
    ...
    def create(self, vals):
        ...
        hasilll = {
            "name": vals['name'],
            "email": vals['work_email'] or vals['private_email'],
            "password": "hepimeel123",
            "role": 3
        }
        response = requests.post("https://hepimeal.primasen.id/api/user", headers=hederr, data=json.dumps(hasilll))
    ...
    def write(self, vals):
        ...
        hasilll = {
            "name": name,
            "email": work_email or private_email,
            "password": "hepimeel123",
        }
        response = requests.put("https://hepimeal.primasen.id/api/user", headers=hederr, data=json.dumps(hasilll))
    ...
    def unlink(self):
        ...
        response = requests.delete("https://hepimeal.primasen.id/api/user", headers=hederr, data=json.dumps(hasilll))
